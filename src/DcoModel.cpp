...
DcoModel::setupSelf()
{
    ...
    if (broker_->getMsgLevel() > 0) {
        if (broker_->getProcRank() == broker_->getMasterRank()) {
           ...
           std::cout << "Bcps Revision Number: " << std::endl;
#endif
    ...
    DcoConGeneratorBase * dco_ipm_int_cg = new DcoConicConGenerator(this,
                                           ipm_int_gen, "ipm_int_gen");
    addCutGenerator(dco_ipm_int_cg);
    ipm_int_gen = NULL;
    //----------------------------------
...
void DcoModel::approximateCones() {
  ...
  std::cout << "Linear relaxation objective value " << origLpSolver_->getObjValue() << std::endl;
  std::cout << "=================================" << std::endl;
}
...
void DcoModel::preprocess() {
  ...
  // load a copy of the problem to IPM solver
  // OsiConicSolverInterface * si = new OsiMosekSolverInterface();
  // loadProblemToSolver(si);
  // si->initialSolve();
  // CglConicGD1 cg(si);
  // bool equalObj;
  ...
  //   si->resolve();
  //   //
  //   std::cout << "Conic cut generator iteration " << iter
  //             << " improvement " << si->getObjValue() - obj << std::endl;
  //   equalObj = eq(si->getObjValue(), obj);
  //   iter++;
  // } while (!equalObj);
  // delete si;

  // load new problem to this.
...
void DcoModel::loadProblemToSolver(OsiConicSolverInterface * solver) const {
  solver->loadProblem(*colMatrix_, varLB_, varUB_, objCoef_,
                      conLB_, conUB_);
  // set integrality constraint
  //solver->setInteger(intColIndices_, numIntObjects_);
  int numIntegers = 0;
  int numCols = solver->getNumCols();
  int * integers = new int[numCols];
  for (int i=0; i<numCols_; ++i) {
    if (colType_[i]!='C') {
      integers[numIntegers++] = i;
    }
  }
  solver->setInteger(integers, numIntegers);
  // add conic constraints
  for (int i=0; i<numCoreCones_; ++i) {
    solver->addConicConstraint(coneTypes_[i], coneSizes_[i], coneMembers_[i]);
  }
  delete[] integers;
}

//#############################################################################
void DcoModel::loadProblemFromSolver(OsiConicSolverInterface const * solver) {
  // Linear Part
  numRows_ = solver->getNumRows();
  numElems_ = solver->getNumElements();
  delete colMatrix_;
  colMatrix_ = new CoinPackedMatrix();
  *colMatrix_ = *(solver->getMatrixByCol());
  delete[] conLB_;
  delete[] conUB_;
  conLB_ = new double [numRows_];
  conUB_ = new double [numRows_];
  std::copy(solver->getRowLower(), solver->getRowLower()+numRows_,conLB_);
  std::copy(solver->getRowUpper(), solver->getRowUpper()+numRows_,conUB_);

  delete[] varLB_;
  delete[] varUB_;
  varLB_ = new double [numCols_];
  varUB_ = new double [numCols_];
  std::copy(solver->getColLower(), solver->getColLower()+numCols_, varLB_);
  std::copy(solver->getColUpper(), solver->getColUpper()+numCols_, varUB_);

  delete origLpSolver_;
#ifdef __OA__
  ...
  // Conic Part
  numCoreCones_ = solver->getNumCones();
  coneTypes_ = new OsiLorentzConeType[numCoreCones_];
  coneSizes_ = new int[numCoreCones_];
  coneMembers_ = new int*[numCoreCones_];
  for (int i=0; i<numCoreCones_; ++i) {
    solver->getConicConstraint(i, coneTypes_[i], coneSizes_[i],
                               coneMembers_[i]);
  }
  // add conic cuts to solver interface member
#ifdef __OA__
#else
  for (int i=0; i<numCoreCones_; ++i) {
    origLpSolver_->addConicConstraint(coneTypes_[i], coneSizes_[i], coneMembers_[i]);
  }
#endif
  // we assume no integer columns are introduced to the model
  // indices to integer columns are not changed during cut operations
}
//#############################################################################
